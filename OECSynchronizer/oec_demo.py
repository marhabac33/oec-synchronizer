#!/usr/bin/python3
#
# Alternate Main program for Open Exoplanet Catalogue (OEC) synchronization
# extension. This will be temoraily available for testing purposes, as main
# program currently downloads/creates to many files to practically deal with
# until xml comparison/update functions are implimented.
...
def run(args, oec_main_path, config):
    ...
    # TESTING: Mapping for testing:
    CATALOGUE_TO_MAP = {'nasa_planets.csv': NASA_CON_MAP,
                        'eu_planets.csv': EXO_EU_MAP}
    ...
    # FOR TESTING:
    csv_files = [(os.path.join(oec_main_path, 'test_data/nasa_planets.csv'),
                  'nasa_planets.csv'),
                 (os.path.join(oec_main_path, 'test_data/eu_planets.csv'),
                  'eu_planets.csv')]
    ...
    for (file_path, file_name) in csv_files:
        ...
        for exoplanet in exoplanet_data:
            ...
            if (exoplanet_name in repo.redo_planets()):
                # If exoplanet alias is already in a file, get that file:
...
def set_auto(args, oec_main_path, config):
    ...
    if (not set_execution(oec_main_path, DEMO_RUN_CMD, config[AUTO_STATE],
